...
def _merge_and_log(ld, sumstats, noun, log):
    ...
    #sumstats.to_csv(noun+'_merged.csv')
...
def _read_ld_sumstats(args, log, fh, alleles=False, dropna=True):
    ...
    #print ref_ld #combines all the ldscore files specified in ref-ld-chr into a single dataframe, extracts SNP and ld columns
    ...
    #print w_ld #combines all the ldscore files specified in w-ld-chr into a single dataframe, extracts SNP and ld columns
    sumstats = _merge_and_log(ref_ld, sumstats, 'reference panel LD', log)
    sumstats = _merge_and_log(sumstats, w_ld, 'regression SNP LD', log)
    w_ld_cname = sumstats.columns[-1] # If only one annotation, the column is LD_weights
    #print w_ld_cname
...
def estimate_h2(args, log):
    ...
    #(self, y, x, w, N, M, n_blocks=200, intercept=None, slow=False, twostep=None, old_weights=False):
    #print chisq
    #print ref_ld
    #print M_annot
